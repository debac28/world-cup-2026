...
`src/main.js` is a hash-router over six tab views in `src/views/` (Today, Matches,
Groups, Bracket, Scorers, Watch). Shared rendering helpers live in `src/lib/components.js` and
...
`src/views/watch.js` is a static-data feature: `public/data/watch-parties.json` (committed;
~55 official FIFA fan zones + bars, each with lat/lng, `startDate`/`endDate`, `official`
flag) is loaded into the model by `data.js` (`model.watchParties`) and shown as a
zip-searchable, distance-sorted list. A top-level match dropdown drives both the venue
date filter and the share text. `src/lib/geo.js` does zip geocoding (free zippopotam.us) +
haversine miles. `src/lib/share.js` powers the two native-share actions — match cards use
**↗ Share** (private, "watch with friends") and venue cards use **📺 Watch here** (public).
Both attach `public/og.png` as a photo via the Web Share API (so WhatsApp/iMessage show an
image + caption with **no URL**), falling back to a `wa.me` text link on desktop. `og.png`
is a one-off generated by `scripts/gen_share.mjs` (needs `@napi-rs/canvas`); rerun only to
change the card art. The OG meta in `index.html` hardcodes the prod Pages URL.
